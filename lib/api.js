...
const app_intf = process.BMW_APP_INTF || 'client';
...
function init_client(init_client_cb = null) {
	...
	// DME
	app.get('/dme/encode-316/:value', (req, res) => {
		DME.encode_316(parseInt(req.params.value));
		res.send(status.dme);
	});
	...
	app.get('/dsp/loudness/:value', (req, res) => {
		DSP.loudness(val2bool(req.params.state));
		res.send(status.dsp);
	});

	app.get('/dsp/m-audio/:value', (req, res) => {
		DSP.m_audio(req.params.value);
		res.send(status.dsp);
	});

	app.get('/dsp/mode/:value', (req, res) => {
		DSP.dsp_mode(req.params.value);
		res.send(status.dsp);
	});

	app.get('/dsp/speaker-test/:value', (req, res) => {
		DSP.speaker_test(val2bool(req.params.value));
		res.send(status.dsp);
	});
	...
	app.get('/gm/doors/sealed/:value', (req, res) => {
		update.status('doors.sealed', val2bool(req.params.value), false);
		res.send(status.doors);
	});
	...
	app.get('/gpio/toggle/:value', (req, res) => {
		gpio.toggle(req.params.value);
		res.send(status.gpio);
	});


	// HDMI
	app.get('/hdmi/cec/command/:value', (req, res) => {
		hdmi_cec.command(req.params.value);
		res.send(status.hdmi.cec);
	});

	app.get('/hdmi/rpi/command/:value', (req, res) => {
		hdmi_rpi.command(req.params.value);
	...
	app.get('/lcm/comfort-turn/:value', (req, res) => {
		update.status('lights.turn.depress_elapsed', 0, false);
		LCM.comfort_turn_flash(val2bool(req.params.value));
		res.send(status.lcm);
	});
	...
	app.get('/lcm/police-lights/:value', (req, res) => {
		LCM.police(val2bool(req.params.value));
		res.send(status.lcm);
	});

	app.get('/lcm/welcome-lights/:value', (req, res) => {
		LCM.welcome_lights(val2bool(req.params.value));
		res.send(status.lcm);
	});
	...
	// POWER
	app.get('/power/:value', (req, res) => {
		power.power(val2bool(req.params.value));
		res.send(status.power);
	});


	// RAD
	app.get('/rad/cassette/:value', (req, res) => {
		RAD.cassette_control(val2bool(req.params.value));
		res.send(status.rad);
	});

	app.get('/rad/power/:value', (req, res) => {
		RAD.audio_power(val2bool(req.params.value));
		res.send(status.rad);
	});

	app.get('/rad/source/:value', (req, res) => {
		RAD.audio_control(val2bool(req.params.value));
		res.send(status.rad);
	});

	app.get('/rad/volume/:value', (req, res) => {
		RAD.volume_control(parseInt(req.params.value));
		res.send(status.rad);
	});
